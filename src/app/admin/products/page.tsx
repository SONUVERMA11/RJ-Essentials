    ...
    return (
        <div>
            ...
            <div className="bg-white rounded-lg p-4 shadow-sm mb-4 flex flex-col md:flex-row gap-3">
                ...
                <button onClick={fetchProducts} className="bg-gray-100 text-gray-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-200">Apply</button>
            </div>
                                    ...
                                    <tr key={p._id} className="border-b border-gray-50 hover:bg-gray-50">
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-3">
                                                {p.images?.[0]?.url ? (
                                                    <img src={p.images[0].url} alt="" className="w-10 h-10 object-contain bg-gray-50 rounded" />
                                                ) : (
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            <p className="text-xs text-gray-400 line-through">{formatPrice(p.mrp)}</p>
                                        </td>
                                        <td className="px-4 py-3">
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1">
                                                <Link href={`/admin/products/${p._id}`} className="p-1.5 text-gray-400 hover:text-[#2874F0] hover:bg-blue-50 rounded" title="Edit">
                                                    <Edit size={15} />
                                                </Link>
                                                <button onClick={() => toggleStatus(p._id, p.status)} className="p-1.5 text-gray-400 hover:text-yellow-600 hover:bg-yellow-50 rounded" title="Toggle Visibility">
                                                    {p.status === 'active' ? <EyeOff size={15} /> : <Eye size={15} />}
                                                </button>
                                                <button onClick={() => handleDelete(p._id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded" title="Delete">
                                                    <Trash2 size={15} />
                                                </button>
                                            </div>
                                        </td>
