        ...
        <div>
            ...
            <div className="flex gap-2 mb-4">
                {['all', 'pending', 'approved'].map(f => (
                    <button key={f} onClick={() => setFilter(f)}
                        className={`px-4 py-2 rounded-md text-sm font-medium ${filter === f ? 'bg-[#2874F0] text-white' : 'bg-card text-muted-foreground border'}`}>
            ...
            <div className="space-y-3">
                {loading ? <p className="text-muted-foreground">Loading...</p> :
                    reviews.length === 0 ? <p className="text-muted-foreground bg-card rounded-lg p-8 text-center shadow-sm">No reviews found</p> :
                        reviews.map(r => (
                            <div key={r._id} className="bg-card rounded-lg p-4 shadow-sm flex gap-4">
