    ...
    return (
        <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-background/80 backdrop-blur-xl shadow-lg shadow-black/5' : 'bg-background/95 backdrop-blur-sm'}`}>
            <div className="max-w-7xl mx-auto px-4">
                ...
                <div className="flex items-center gap-3 md:gap-5 h-14 md:h-16">
                    ...
                    <div ref={searchRef} className="flex-1 max-w-xl relative hidden md:block">
                        <form onSubmit={handleSearch} className="relative">
                            <Search size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground/50" />
                            <input
                                type="text"
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                placeholder="Search for products, brands and more"
                                className="w-full h-10 pl-10 pr-4 text-sm bg-muted/40 hover:bg-muted/60 focus:bg-muted/60 text-foreground rounded-full focus:outline-none focus:ring-2 focus:ring-[#2874F0]/30 placeholder:text-muted-foreground/60 transition-all duration-200"
                                id="search-input"
                            />
                        </form>
                        {renderSuggestions()}
                ...
                <div ref={mobileSearchRef} className="md:hidden pb-2.5 relative">
                    <form onSubmit={handleSearch} className="relative">
                        ...
                        <input
                            ...
                            className="w-full h-9 pl-9 pr-4 text-sm bg-muted/40 text-foreground rounded-full focus:outline-none focus:ring-2 focus:ring-[#2874F0]/30 placeholder:text-muted-foreground/60 transition-all duration-200"
                ...
                {/* Category Nav — hidden on product pages */}
                <div className={`hidden md:flex items-center gap-0.5 pb-2 overflow-x-auto scrollbar-hide -mx-1 ${isProductPage ? '!hidden' : ''}`}>
                    {CATEGORIES.map((cat) => {
                        const Icon = cat.icon;
                        return (
                            <Link
